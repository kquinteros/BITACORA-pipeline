...
def bitacora_output(wildcards):
    return expand("{outdir}/GeMoMa/{sample}/{dv}/{db}tblastn_parsed_list_genomic_positions_nogff_filtered.bed", sample=genomes['Sample'], db=proteins['DB'], outdir=config["outdir"])
