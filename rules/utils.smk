...
#def bitacora_annot(wildcards):
#    return expand("{outdir}/{dir}/{sample}/{db}/{db}_genomic_{type}_trimmed.{file}", dir=["Proximity", "GeMoMa"] ,sample=genomes['Sample'], db=proteins['DB'], type = ["proteins", "genes"], ext=["fasta","gff3"] outdir=config["outdir"])
...
def Additional_filter_gff(wildcards):
    return expand("{outdir}/GeMoMa/{sample}/{db}/{db}_genomic_genes_trimmed_idseqsclustered.gff3", sample=genomes['Sample'], db=proteins['DB'], outdir=config["outdir"])
